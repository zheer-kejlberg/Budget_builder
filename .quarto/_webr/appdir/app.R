...
server <- function(input, output, session) {
  ...
  # Columns: Site, Post name, Category, Note,
  #          [period amount cols], [FTE period cols],
  #          " " (spacer), [status total cols]
  # Followed by: per-site total rows, blank separator rows, grand total row.
  # ---------------------------------------------------------------------------
  build_summary_table_data <- function(period_choice, posts_data, budget_start, budget_end,
    ...
    if (!nrow(posts_data)) {
      return(list(
        data        = tibble(Site = character(), `Post name` = character(),
    ...
    # Build Period label, then aggregate monthly rows to (id, Period)
    combined <- long %>%
      left_join(meta, by = "id") %>%
      mutate(
        Period = case_when(
    ...
    # Per-post-period: sum amount, compute period-specific FTE
    ...
    # Post-level metadata (constant across periods)
    ...
    # Determine ordered period names
    all_periods <- sort(unique(post_period$Period))

    # Pivot amounts wide
    amounts_wide <- post_period %>%
      select(id, Period, amount) %>%
      tidyr::pivot_wider(names_from = Period, values_from = amount,
                         values_fill = NA_real_)

    # Pivot FTE wide (rename cols to "FTE <period>")
    fte_wide <- post_period %>%
      select(id, Period, period_fte) %>%
      tidyr::pivot_wider(names_from = Period, values_from = period_fte,
                         values_fill = NA_real_) %>%
    ...
    # Ensure all expected period/FTE cols present (in case a post spans only part)
    ...
    statuses_present <- intersect(status_order,
                                  unique(post_meta$application_status))
    status_cols <- statuses_present

    # Per-post status amount totals (sum across all periods for that post)
    ...
    for (st in status_cols) {
      st_totals <- post_status_totals %>%
        ...
        select(id, total)
      df <- df %>%
        left_join(st_totals %>% rename(!!st := total), by = "id")
    }

    # Drop id and application_status helper cols; reorder
    df <- df %>%
      select(-id, -application_status,
             Site, `Post name`, Category, Note,
             all_of(period_cols), all_of(fte_cols),
             " ", all_of(status_cols))

    # Build numeric cols vector (all period + fte + status cols)
    num_cols <- c(period_cols, fte_cols, status_cols)

    # ----- build structured rows: posts + site totals + separators + grand total -----
    sites_vec <- unique(df$Site)

    rows_list    <- list()
    site_totals  <- list()  # collect site-total rows for grand-total summing

    for (site in sites_vec) {
      site_posts <- df %>% filter(Site == site)
      rows_list <- c(rows_list, list(site_posts))

      # Site total row: sum all numeric cols across site's posts
      st_row <- site_posts %>%
        summarise(across(all_of(num_cols), ~ sum(.x, na.rm = TRUE)))
      st_row <- bind_cols(
        tibble(Site = site, `Post name` = "Site total",
      ...
      # Blank separator
      blank_row <- df[0, ][1, ]  # one NA row with same schema
      blank_row[1, ] <- NA
      blank_row$Site <- NA_character_
      rows_list <- c(rows_list, list(blank_row))
    }

    # Grand total row: sum of site-total rows only
    site_totals_df <- bind_rows(site_totals)
    grand_row <- site_totals_df %>%
      summarise(across(all_of(num_cols), ~ sum(.x, na.rm = TRUE)))
    grand_row <- bind_cols(
      tibble(Site = NA_character_, `Post name` = "Grand total",
             Category = NA_character_, Note = NA_character_),
      grand_row,
      tibble(" " = NA_character_)
    )
  ...
  output$summary_form_table <- renderDT({
    ...
    amount_cols <- c(period_cols, status_cols)
    round_cols  <- c(period_cols, fte_cols, status_cols)

    # Row-type sentinels (used in rowCallback)
    dt <- datatable(
      df,
      rownames  = FALSE,
      selection = "none",
      options = list(
        ...
        rowCallback = JS(
          "function(row, data) {",
          "  var site  = typeof data[0] === 'string' ? data[0] : '';",
          "  var pname = typeof data[1] === 'string' ? data[1] : '';",
          "  if (pname === 'Site total') {",
          "    $(row).css({'font-weight':'700', 'background-color':'#f1f1f1'});",
          "  } else if (pname === 'Grand total') {",
          "    $(row).css({'font-weight':'700', 'background-color':'#e4e4e4'});",
          "  } else if (site === '' || site === null || (typeof site === 'object' && site === null)) {",
          "    $(row).css({'background-color':'#ffffff', 'line-height':'8px'});",
          "    $('td', row).text('');",
          "  }",
          "}"
        )
    ...
    if (length(round_cols) > 0) {
      round_cols_present <- intersect(round_cols, names(df))
    ...
    if (length(status_cols) > 0) {
      status_cols_present <- intersect(status_cols, names(df))
      if (length(status_cols_present) > 0)
        dt <- dt %>% formatStyle(columns = status_cols_present, fontWeight = "bold")
    }
  ...
  observeEvent(input$edit_selected, {
    ...
    # If salary fields are populated, verify the formula matches what would be generated
    # If it matches, show salary mode. If it doesn't match (manually edited), show formula mode.
    selected_mode <- "function"  # default
    if (nzchar(rv$salary_selected_draft) && nzchar(rv$salary_field_selected_draft)) {
      # Salary fields are populated - check if formula matches what we'd generate
    ...
    # If not salary mode, check for other modes
    if (selected_mode != "salary") {
      if (identical(row$value_mode[[1]], "variable")) {
        selected_mode <- "variable"
  ...
  observeEvent(input$add_or_update, {
    ...
    if (val_mode == "salary") {
      ...
      # If apply_inflation is checked, wrap with apply_inflation()
      if (isTRUE(apply_inflation_flag)) {
        generated_function_expr <- paste0("apply_inflation(", generated_function_expr, ")")
      }
      # For validation and storage, convert to function mode with the generated formula
      actual_value_mode <- "function"
    } else if (val_mode == "function") {
      # For function mode, wrap with apply_inflation() if checkbox is checked
      user_function_expr <- if (is_editing) rv$function_expr_draft else input$function_expr
      if (isTRUE(apply_inflation_flag) && !is.null(user_function_expr) && nzchar(user_function_expr)) {
        generated_function_expr <- paste0("apply_inflation(", user_function_expr, ")")
      }
    }
    ...
    new_row <- tibble(
      ...
      constant_expr = rv$constant_expr_draft,
      function_expr = if (nzchar(generated_function_expr)) generated_function_expr else ifelse(is.null(input$function_expr) || !nzchar(input$function_expr), rv$function_expr_draft, input$function_expr),
