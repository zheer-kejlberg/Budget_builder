...
resolve_post_values <- function(post_row, salaries_lookup = data.frame(), inflation_pct = 0, fte_monthly_total = NULL, salaries_tbl = NULL, budget_start = NULL, all_posts_tbl = NULL) {
  ...
  get_site_total <- function(sites) {
    if (is.null(all_posts_tbl) || !nrow(all_posts_tbl)) {
      return(if (unit == "month") rep(0, n_months) else rep(0, n_years))
    }
    
    sites <- as.character(sites)
    current_post_id <- as.numeric(post_row$id[[1]])
    
    # Filter to posts in the specified sites (excluding current post and posts that use sum_total to prevent circular refs)
    site_posts <- all_posts_tbl %>%
      filter(
        center %in% sites,
        id != current_post_id,
        value_mode == "variable" | !grepl("sum_total", function_expr, ignore.case = TRUE)
      )
    ...
    # Always work with monthly values internally, then convert at the end if needed
    result_monthly <- rep(0, n_months)
    
    for (i in seq_len(nrow(site_posts))) {
      other_post <- site_posts[i, ]
      other_mode <- as.character(other_post$value_mode[[1]])
      other_unit <- as.character(other_post$value_unit[[1]])
      other_id <- as.numeric(other_post$id[[1]])
      
      tryCatch({
        # Evaluate the post to get its amounts (works for all modes that aren't using sum_total)
        ...
        # Extract the value column and its corresponding months
        # resolve_post_values ALWAYS returns monthly values
        if (!is.null(resolved_tibble) && nrow(resolved_tibble) > 0) {
          ...
          # Align other_post's values to the calling post's date range
          # Create a vector for this post's contribution spanning the full calling period
          this_contribution <- rep(0, n_months)
          
          # Find which indices in the calling post's months correspond to other_post's months
          for (j in seq_len(length(other_months))) {
            calling_idx <- which(months == other_months[j])
            if (length(calling_idx) > 0) {
              this_contribution[calling_idx[1]] <- other_amounts_monthly[j]
            }
          }
          
          # Add to result
          result_monthly <- result_monthly + this_contribution
        }
      }, error = function(e) {
        # If a post fails to evaluate, skip it silently
        NULL
      })
    }
    
    # Convert back to the calling post's unit if needed
    if (unit == "year") {
      # Aggregate monthly to yearly
      result_yearly <- vapply(seq_len(n_years), function(j) {
        idx_start <- (j - 1) * 12 + 1
        idx_end <- min(j * 12, n_months)
        sum(result_monthly[idx_start:idx_end], na.rm = TRUE)
      }, numeric(1))
  ...
  extra_env <- list(
    ...
    sum_total = function(site = current_site) {
      get_site_total(site)
    },
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        column(12, actionButton("add_or_update", "Add budget post", class = "btn-success", style = "width: 100%;"))
      ),
      uiOutput("form_error"),
...
server <- function(input, output, session) {
  ...
  reset_form <- function() {
    ...
    updateDateRangeInput(session, "post_date_range", start = NA, end = NA)
  ...
  # Render unit radio buttons - only allow "Month" for formula mode
  output$value_unit_ui <- renderUI({
    mode <- input$value_mode
    if (mode == "function") {
      # Formula mode: only allow monthly
  ...
  # When mode changes to "function", ensure value_unit is set to "month"
  observeEvent(input$value_mode, {
    if (input$value_mode == "function") {
      updateRadioButtons(session, "value_unit", selected = "month")
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$add_or_update, {
    ...
    if (is.null(input$application_status) || !nzchar(trimws(input$application_status))) missing_fields <- c(missing_fields, "Application status")
  ...
  observeEvent(input$show_formula_help, {
    showModal(modalDialog(
      ...
      tags$ul(
        ...
        tags$li(tags$b("sum_total(site)"), " - function returning sum of all amounts across posts within each chosen period (month, year). By default sums across all posts in the current post's site, but this can be changed with the site argument; e.g., sum_total(site = 'Main'), sum_total(site = all_sites)."),
