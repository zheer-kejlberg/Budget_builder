...
server <- function(input, output, session) {
  ...
  reset_form <- function() {
    ...
    rv$sum_multiplier_draft <- "1"
    rv$sum_sites_draft <- character()
    rv$sum_statuses_draft <- character()
    rv$sum_posts_draft <- character()
    ...
    updateTextInput(session, "sum_multiplier", value = "1")
    # For sum mode, apply smart defaults for new posts
    updateSelectizeInput(session, "sum_sites", selected = input$center %||% character())
    updateSelectizeInput(session, "sum_statuses", selected = "Applied for")
  ...
  observeEvent(input$edit_selected, {
    ...
    rv$sum_multiplier_draft <- if (!is.null(row$sum_multiplier) && nzchar(as.character(row$sum_multiplier))) {
      as.character(row$sum_multiplier)
    } else {
      "1"
    }
    
    rv$sum_sites_draft <- if (!is.null(row$sum_sites) && nzchar(as.character(row$sum_sites))) {
      strsplit(as.character(row$sum_sites), "||", fixed = TRUE)[[1]]
    } else {
      character()
    }
    
    rv$sum_statuses_draft <- if (!is.null(row$sum_statuses) && nzchar(as.character(row$sum_statuses))) {
      strsplit(as.character(row$sum_statuses), "||", fixed = TRUE)[[1]]
    } else {
      character()
    }
    
    rv$sum_posts_draft <- if (!is.null(row$sum_posts) && nzchar(as.character(row$sum_posts))) {
      strsplit(as.character(row$sum_posts), "||", fixed = TRUE)[[1]]
    } else {
      character()
    }
    ...
    selected_mode <- if (identical(row$value_mode, "variable")) {
      "variable"
    } else if (identical(row$value_mode, "sum")) {
      "sum"
    } else {
      "function"
    }
  ...
  observeEvent(input$add_or_update, {
    ...
    new_row <- tibble(
      ...
      sum_multiplier = ifelse(is.null(input$sum_multiplier), rv$sum_multiplier_draft, input$sum_multiplier),
      sum_sites = ifelse(is.null(input$sum_sites), rv$sum_sites_draft, paste(input$sum_sites, collapse = "||")),
      sum_statuses = ifelse(is.null(input$sum_statuses), rv$sum_statuses_draft, paste(input$sum_statuses, collapse = "||")),
      sum_posts = ifelse(is.null(input$sum_posts), rv$sum_posts_draft, paste(input$sum_posts, collapse = "||")),
      note = input$note,
      needs_amendment = FALSE,
      application_status = if (!is.null(input$application_status)) input$application_status else "Applied for"
    )
