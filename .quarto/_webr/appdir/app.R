...
get_post_amendment_fields <- function(post_row, budget_start, budget_end, category_registry, site_registry) {
  ...
  if (post_row$start_date < budget_start) fields <- c(fields, "post_start")
  if (post_row$end_date > budget_end) fields <- c(fields, "post_end")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      dateInput("post_start", required_label("Post start")),
      dateInput("post_end", required_label("Post end")),
...
server <- function(input, output, session) {
  ...
  refresh_post_field_labels <- function() {
    ...
    updateDateInput(session, "post_start", label = amend_label("Post start", show_amend = is_amended_field("post_start"), required = TRUE))
    updateDateInput(session, "post_end", label = amend_label("Post end", show_amend = is_amended_field("post_end"), required = TRUE))
  }
