...
check_import_post_issues <- function(post_row, existing_posts, salaries_lookup = data.frame(), inflation_pct = 0, salaries_tbl = NULL, site_registry = NULL, category_registry = NULL) {
  ...
  if (post_row$value_mode %in% c("function", "salary", "numeric", "sum") &&
      !(post_row$value_mode == "function" && !nzchar(trimws(post_row$function_expr)))) {
...
resolve_post_values <- function(post_row, salaries_lookup = data.frame(), inflation_pct = 0, fte_monthly_total = NULL, salaries_tbl = NULL, budget_start = NULL, all_posts_tbl = NULL) {
  ...
  extra_env <- list(
    ...
    fte = function(site = current_site) {
      monthly_vals <- get_site_fte(site)
      if (unit == "year") {
        vapply(seq_len(n_years), function(i) {
          idx_start <- (i - 1) * 12 + 1
          idx_end <- min(i * 12, n_months)
          sum(monthly_vals[idx_start:idx_end], na.rm = TRUE)
        }, numeric(1))
      } else {
        monthly_vals
      }
  ...
  if (mode == "constant") {
    # Legacy mode — treated as numeric single-value
    ...
    vec <- unlist(post_row$value_vector, use.names = FALSE)
    if (length(vec) == 0) {
      # Single-value sub-mode: use constant_expr
      constant_expr_safe <- if (is.null(post_row$constant_expr) || !nzchar(trimws(post_row$constant_expr))) "0" else post_row$constant_expr
      scalar <- safe_eval_expr(constant_expr_safe, fte = fte, n = n_months, extra_env = extra_env)
      if (length(scalar) != 1L || !is.numeric(scalar) || is.na(scalar)) {
        stop("Numeric (single) amount must resolve to one numeric value.", call. = FALSE)
      }
      values <- rep(as.numeric(scalar), if (unit == "month") n_months else n_years)
    } else {
      # Multi-value sub-mode: use value_vector
      values <- vec
      if (!is.numeric(values)) {
        stop("Numeric (multi) amounts must be numeric.", call. = FALSE)
      }
    }
  } else if (mode == "salary") {
    # Salary mode: look up salary by identifier stored in function_expr
    salary_id <- as.character(post_row$function_expr[[1]])
    if (is.null(salary_id) || !nzchar(trimws(salary_id))) {
      stop("Salary mode requires a salary calculation to be selected.", call. = FALSE)
    }
    sal_obj <- salary_objs[[salary_id]]
    if (is.null(sal_obj)) {
      stop(paste0("Salary identifier '", salary_id, "' not found."), call. = FALSE)
    }
    sal_value <- if (unit == "year") sal_obj$total_y else sal_obj$total
    fte_val <- if (is.na(fte)) 1 else as.numeric(fte)
    n_needed <- if (unit == "year") n_years else n_months
    values <- rep(as.numeric(sal_value) * fte_val, n_needed)
...
serialize_inactive_posts <- function(posts_tbl) {
  ...
  posts_tbl %>%
    ...
    mutate(
      ...
      apply_inflation_flag = as.logical(apply_inflation_flag)
    )
}
...
parse_inactive_posts <- function(posts_tbl) {
  ...
  # Add apply_inflation_flag if missing (backward compatibility)
  if (!"apply_inflation_flag" %in% names(posts_tbl)) {
    posts_tbl$apply_inflation_flag <- FALSE
  }
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("value_mode", "How should amount be defined?", choices = c("Formula" = "function", "Salary" = "salary", "Numeric" = "numeric", "Sum of other posts" = "sum"), selected = "function"),
...
server <- function(input, output, session) {
  rv <- reactiveValues(
    ...
    apply_inflation_flag_draft = FALSE,
    numeric_sub_mode_draft = "single",
    numeric_single_value_draft = 0,
    salary_id_draft = ""
  )
  ...
  reset_form <- function() {
    ...
    rv$apply_inflation_flag_draft <- FALSE
    rv$numeric_sub_mode_draft <- "single"
    rv$numeric_single_value_draft <- 0
    rv$salary_id_draft <- ""
    ...
    updateCheckboxInput(session, "apply_inflation_post", value = FALSE)
  ...
  observeEvent(input$template_name, {
    ...
    selected_mode <- if (identical(tpl$mode, "variable") || identical(tpl$mode, "constant") || identical(tpl$mode, "numeric")) {
      "numeric"
    } else if (identical(tpl$mode, "salary")) {
      "salary"
    } else if (identical(tpl$mode, "sum")) {
      "sum"
    } else {
      "function"
    }
    if (identical(tpl$mode, "variable")) {
      rv$numeric_sub_mode_draft <- "multi"
      rv$variable_defaults <- if (is.null(tpl$values[[1]])) numeric() else as.numeric(tpl$values[[1]])
    } else if (identical(tpl$mode, "constant") || identical(tpl$mode, "numeric")) {
      rv$numeric_sub_mode_draft <- "single"
      rv$numeric_single_value_draft <- suppressWarnings(as.numeric(tpl$constant_expr)) %||% 0
    }
  ...
  # Render unit radio buttons — hidden for formula/sum/salary, shown for numeric
  output$value_unit_ui <- renderUI({
    mode <- input$value_mode
    if (mode == "function" || mode == "sum" || mode == "salary") {
      # These modes do not expose a unit selector
      NULL
    } else {
      # Numeric mode: allow both month and year
      radioButtons("value_unit", "Input amount per", choices = c("Month" = "month", "Year" = "year"), selected = isolate(rv$value_unit_draft %||% "month"), inline = TRUE)
    }
  })

  # When mode changes to "function", "sum", or "salary", ensure value_unit is reset to "month"
  observeEvent(input$value_mode, {
    if (input$value_mode %in% c("function", "sum", "salary")) {
      updateRadioButtons(session, "value_unit", selected = "month")
    }
  }, ignoreInit = TRUE)
  ...
  output$value_inputs_ui <- renderUI({
    ...
    inflation_cb <- checkboxInput("apply_inflation_post", "Apply inflation", value = isolate(rv$apply_inflation_flag_draft))
    ...
    if (mode == "function") {
      ...
      salary_choices <- if (nrow(rv$salaries) > 0) setNames(rv$salaries$identifier, rv$salaries$name) else character(0)
      current_salary_id <- isolate(rv$salary_id_draft)
      tagList(
        if (length(salary_choices) == 0) {
          helpText("No salary calculations defined. Go to the 'Salary calculations' tab to add one.")
        } else {
          selectInput("salary_id_post", "Salary calculation", choices = salary_choices, selected = current_salary_id)
        },
        inflation_cb
      )
    } else if (mode == "sum") {
      ...
      sub_mode <- isolate(rv$numeric_sub_mode_draft)
      unit_val <- isolate(input$value_unit %||% rv$value_unit_draft %||% "month")

      if (sub_mode == "single") {
        tagList(
          radioButtons("numeric_sub_mode", "Input type",
                       choices = c("Single value" = "single", "Multiple values" = "multi"),
                       selected = "single", inline = TRUE),
          numericInput("numeric_single_value", "Amount per period",
                       value = isolate(rv$numeric_single_value_draft), step = 100, min = 0),
          inflation_cb
        )
      } else {
        n_vals <- isolate(required_value_count())
        ...
        tagList(
          radioButtons("numeric_sub_mode", "Input type",
                       choices = c("Single value" = "single", "Multiple values" = "multi"),
                       selected = "multi", inline = TRUE),
          helpText(paste("Provide", n_vals, ifelse(unit_val == "year", "yearly", "monthly"), "amounts.")),
          lapply(seq_len(n_vals), function(i) {
            unit_label <- ifelse(unit_val == "year", "Year", "Month")
            numericInput(
  ...
  # Sync numeric sub-mode radio to draft and re-render inputs
  observeEvent(input$numeric_sub_mode, {
    rv$numeric_sub_mode_draft <- input$numeric_sub_mode
    rv$value_inputs_refresh <- rv$value_inputs_refresh + 1L
  }, ignoreInit = TRUE)

  # Sync apply inflation checkbox to draft
  observeEvent(input$apply_inflation_post, {
    rv$apply_inflation_flag_draft <- isTRUE(input$apply_inflation_post)
  }, ignoreInit = TRUE)

  # Sync salary selection to draft
  observeEvent(input$salary_id_post, {
    rv$salary_id_draft <- input$salary_id_post %||% ""
  }, ignoreInit = TRUE)

  # Sync numeric single value to draft
  observeEvent(input$numeric_single_value, {
    rv$numeric_single_value_draft <- suppressWarnings(as.numeric(input$numeric_single_value)) %||% 0
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$edit_selected, {
    ...
    # Restore apply inflation flag
    rv$apply_inflation_flag_draft <- isTRUE(row$apply_inflation_flag[[1]])

    # Restore salary/numeric mode drafts and map legacy modes
    raw_mode <- as.character(row$value_mode[[1]] %||% "function")
    vec_vals <- unlist(row$value_vector[[1]], use.names = FALSE)
    if (raw_mode == "variable") {
      # Legacy variable mode → numeric multi
      rv$numeric_sub_mode_draft <- "multi"
      rv$variable_defaults <- vec_vals
    } else if (raw_mode == "constant") {
      # Legacy constant mode → numeric single
      rv$numeric_sub_mode_draft <- "single"
      rv$numeric_single_value_draft <- suppressWarnings(as.numeric(row$constant_expr[[1]])) %||% 0
    } else if (raw_mode == "numeric") {
    ...
    } else if (raw_mode == "salary") {
      rv$salary_id_draft <- as.character(row$function_expr[[1]] %||% "")
    }
    ...
    selected_mode <- if (raw_mode %in% c("variable", "constant", "numeric")) {
      "numeric"
    } else if (raw_mode == "salary") {
      "salary"
    } else if (raw_mode == "sum") {
      "sum"
    } else {
      "function"
    }
    rv$value_mode_draft <- selected_mode
    updateSelectInput(session, "value_mode", selected = selected_mode)
    ...
    updateCheckboxInput(session, "apply_inflation_post", value = rv$apply_inflation_flag_draft)
  ...
  observeEvent(input$add_or_update, {
    ...
    vec <- numeric()
    current_mode <- if (is_editing) rv$value_mode_draft else input$value_mode

    if (identical(current_mode, "numeric")) {
      sub_mode <- rv$numeric_sub_mode_draft
      if (identical(sub_mode, "multi")) {
    ...
    # Resolve constant_expr and function_expr based on mode
    resolved_constant_expr <- if (identical(current_mode, "numeric") && identical(rv$numeric_sub_mode_draft, "single")) {
      as.character(rv$numeric_single_value_draft %||% 0)
    } else {
      rv$constant_expr_draft
    }

    resolved_function_expr <- if (identical(current_mode, "salary")) {
      rv$salary_id_draft %||% (input$salary_id_post %||% "")
    } else {
      ifelse(is.null(input$function_expr) || !nzchar(input$function_expr), rv$function_expr_draft, input$function_expr)
    }

    # Determine value_unit: formula/sum/salary always monthly
    resolved_unit <- if (current_mode %in% c("function", "sum", "salary")) {
      "month"
    } else if (is_editing) {
      rv$value_unit_draft
    } else {
      input$value_unit %||% "month"
    }
    ...
    new_row <- tibble(
      ...
      value_mode = current_mode,
      value_unit = resolved_unit,
      constant_expr = resolved_constant_expr,
      ...
      import_issues = "",
      apply_inflation_flag = rv$apply_inflation_flag_draft
    )
