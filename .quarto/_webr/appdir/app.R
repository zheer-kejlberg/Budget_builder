...
make_default_salaries <- function() {
  ...
  phd <- calc_salary_fields(439707, "year",  "percentage", 18.07,  33.3,    wage_supplement = 18849.77, holiday_rate_pct = 0, subtract_holiday = FALSE)
  tibble(
    id                                = 1L:3L,
    ...
    base_salary                       = c(12000, 48500, 439707),
    pension_mode                      = c("percentage", "percentage", "percentage"),
    pension_value                     = c(0, 19.36, 18.07),
    own_pension_pct                   = c(0, 33.3, 33.3),
    wage_supplement                   = c(0, 0, 18849.77),
    holiday_rate                      = c(0, 12.5, 0),
    subtract_holiday                  = c(FALSE, TRUE, FALSE),
...
make_salary_lookup <- function(salaries_tbl) {
  ...
  out <- salaries_tbl %>%
    ...
    transmute(
      ...
      total_plus_holiday = total_plus_holiday_salary_monthly,
      total_plus_holiday_y = total_plus_holiday_salary_yearly,
      total = total_salary_monthly,
...
# Issue #4: Create salary environment objects for formula access (identifier$total style)
make_salary_env_objects <- function(salaries_tbl) {
  ...
  for (i in seq_len(nrow(active_salaries))) {
    ...
    salary_obj <- data.frame(
      base = row$base_salary_monthly,
      pension = row$pension_amount_monthly,
      own_pension = row$own_pension_amount_monthly,
      holiday_base = row$holiday_allowance_total_monthly,
      holiday = row$holiday_allowance_monthly,
      total_plus_holiday = row$total_plus_holiday_salary_monthly,
      total_plus_holiday_y = row$total_plus_holiday_salary_yearly,
      total = row$total_salary_monthly,
...
make_default_templates <- function() {
  tibble(
    ...
    category = c("", "Salary, research year", "Salary, Ph.D. students"),
    center = rep("", 3),
    mode = c("function", "function", "function"),
    unit = c("month", "month", "month"),
    constant_expr = rep("0", 3),
    function_expr = c("apply_inflation(clinical_researcher$total)*FTE", "research_year$total", "apply_inflation(phd_student$total)"),
    fte = c(NA_real_, 1, 1),
    note = c("", "", ""),
...
resolve_post_values <- function(post_row, salaries_lookup = data.frame(), inflation_pct = 0, fte_monthly_total = NULL, salaries_tbl = NULL, budget_start = NULL, all_posts_tbl = NULL) {
  ...
  # Issue #4: Generate salary objects for identifier$total access
  salary_objs <- list()
  ...
  if (mode == "constant") {
    ...
    # Apply inflation wrapping if flag is set
    if (isTRUE(post_row$apply_inflation) && nzchar(trimws(function_expr_safe))) {
      function_expr_safe <- paste0("apply_inflation(", function_expr_safe, ")")
    }
    ...
    # Salary mode: generate formula on-the-fly from salary_selected and salary_field_selected
    sal_selected <- post_row$salary_selected
    sal_field <- post_row$salary_field_selected
    
    if (!nzchar(sal_selected) || !nzchar(sal_field)) {
      stop("Salary post missing salary_selected or salary_field_selected.", call. = FALSE)
    }
    
    # Look up the salary identifier from the salaries table
    salary_identifier <- NA_character_
    if (!is.null(salaries_tbl) && nrow(salaries_tbl) > 0) {
      sal_row <- salaries_tbl %>% filter(name == sal_selected, !is_deleted)
      if (nrow(sal_row) > 0) {
        salary_identifier <- sal_row$identifier[1]
      }
    }
    
    if (is.na(salary_identifier) || !nzchar(salary_identifier)) {
      stop(paste0("Salary '", sal_selected, "' not found or has been deleted."), call. = FALSE)
    }
    
    # Generate formula: salary_identifier$field
    generated_formula <- paste0(salary_identifier, "$", sal_field)
    
    # Apply multiply by FTE if checked
    if (isTRUE(post_row$multiply_by_fte)) {
      generated_formula <- paste0(generated_formula, " * FTE")
    }
    
    # Apply inflation wrapping if flag is set
    if (isTRUE(post_row$apply_inflation)) {
      generated_formula <- paste0("apply_inflation(", generated_formula, ")")
    }
    
    n_values <- if (unit == "year") n_years else n_months
    values <- safe_eval_expr(generated_formula, fte = fte, n = n_values, extra_env = extra_env)
    if (!is.numeric(values)) {
      stop("Salary formula must resolve to numeric values.", call. = FALSE)
    }
  } else if (mode == "sum") {
...
serialize_posts <- function(posts_tbl) {
  if (!"apply_inflation" %in% names(posts_tbl)) {
    posts_tbl$apply_inflation <- FALSE
  }
  posts_tbl %>%
    mutate(
...
parse_inactive_posts <- function(posts_tbl) {
  ...
  required <- c(
    ...
    "value_vector", "salary_selected", "salary_field_selected", "numeric_input_mode", "multiply_by_fte", 
    "apply_inflation", "sum_multiplier", "sum_sites", "sum_statuses", "sum_posts", "note", "needs_amendment", "application_status", "import_issues"
  )
  ...
  posts_tbl %>%
    mutate(
      ...
      apply_inflation = as.logical(apply_inflation),
...
parse_posts <- function(posts_tbl) {
  ...
  required <- c(
    ...
    "fte", "value_mode", "value_unit", "constant_expr", "function_expr",
    "value_vector", "salary_selected", "salary_field_selected", "numeric_input_mode", "multiply_by_fte", 
    "apply_inflation", "sum_multiplier", "sum_sites", "sum_statuses", "sum_posts", "note", "needs_amendment", "application_status"
  )
...
ui <- fluidPage(
  ...
  tags$head(tags$script(HTML(
    ...
    Shiny.addCustomMessageHandler('save-pdf', function(payload) {
      ...
      // Draw body rows with alternating row colors for site separation
      var yy = totalHeadH;
      bodyRows.forEach(function(row, rowIdx) {
        var col = 0;
        var shouldHighlight = rowIdx % 2 === 0;
        row.forEach(function(cell) {
          var span = Math.min(cell.cs || 1, nCols - col);
          var cw2 = 0;
          for (var k = 0; k < span; k++) { if (col + k < nCols) cw2 += colWidths[col + k]; }
          var bg = cell.bg;
          if (bg === 'rgb(255,255,255)' && shouldHighlight) {
            bg = '#f9f9f9';
          }
          drawCell(colX[col], cw2, yy, BODY_ROW_H, cell.text, {
            bg: bg, bold: cell.bold, italic: cell.italic, color: cell.color, align: cell.align
          });
          col += span;
        });
        yy += BODY_ROW_H;
      ...
      // Wait for plots to be ready
      setTimeout(function() {
        console.log('Starting PDF generation...');
        
        // Convert table canvas to image
        ...
        Promise.all([
          ...
          // Page 2: Applied for visualization
          doc.addPage();
          doc.setFontSize(12);
      ...
      }, 2500); // Wait 2.5 seconds for plots and table to render
    });
    "
  ))),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("value_mode", "How should amount be defined?", choices = c("Numeric" = "variable", "Salary" = "salary", "Sum of other posts" = "sum", "Custom formula" = "function"), selected = "function"),
      uiOutput("value_unit_ui"),
      uiOutput("value_inputs_ui"),
      uiOutput("apply_inflation_ui"),
...
server <- function(input, output, session) {
  rv <- reactiveValues(
    ...
    next_template_id = 5L,
    next_id = 1L,
    next_salary_id = max(make_default_salaries()$id, na.rm = TRUE) + 1L,
  ...
  reset_form <- function(skip_template_name = FALSE) {
    ...
    updateCheckboxInput(session, "apply_inflation", value = FALSE)
  ...
  output$export_progress <- renderUI({
    if (!isTRUE(rv$export_in_progress)) return(NULL)
    tags$div(
      class = "export-progress-wrap",
      tags$div(
        class = "progress",
  ...
  output$salary_table <- renderDT({
    salary_display <- rv$salaries %>%
      mutate(
        Actions = sapply(seq_len(nrow(rv$salaries)), function(i) {
          id <- rv$salaries$id[i]
          is_default <- rv$salaries$is_default[i]
          is_deleted <- rv$salaries$is_deleted[i]
          is_edited <- rv$salaries$is_edited[i]
          if (is_deleted) {
            sprintf('<button class="salary-restore-btn" data-id="%d" title="Restore" style="background:none;border:none;cursor:pointer;color:#007bff;font-size:16px;padding:0 4px;line-height:1;display:inline-block;"><i class="fas fa-undo"></i></button>', id)
          } else {
            buttons <- ""
            if (is_default && is_edited) {
  ...
  output$salary_identifier_help <- renderText({
    if (!nrow(rv$salaries)) {
      return("Formula helper: after you add salaries, reference them like phd_student$total.")
    }
    ids <- paste(rv$salaries$identifier, collapse = ", ")
    first_id <- rv$salaries$identifier[[1]]
    paste0("Formula helper: identifiers are ", ids, ". Example: apply_inflation(", first_id, "$total)")
  })
  ...
  observeEvent(input$salary_add_or_update, {
    ...
    # Check for duplicate salary names
    existing_names <- if (is_new) {
      rv$salaries %>% filter(!is_deleted) %>% pull(name)
    } else {
      rv$salaries %>% filter(!is_deleted, id != rv$editing_salary_id) %>% pull(name)
    }
    if (trimws(input$salary_name) %in% existing_names) {
      rv$salary_error_text <- "A salary with this name already exists. Salary names must be unique."
      return()
    }
    ...
    calc <- tryCatch(
      calc_salary_fields(
        ...
        wage_supplement = input$salary_wage_supplement,
        holiday_rate_pct = input$salary_holiday_rate,
        subtract_holiday = input$salary_subtract_holiday
      ),
      error = function(e) {
    ...
    row <- tibble(
      ...
      is_edited = if (is_new) {
        ...
        # When editing a default salary, mark as edited so revert button appears
        old_is_default <- rv$salaries %>% filter(id == sid) %>% pull(is_default) %>% .[1]
        if (old_is_default) TRUE else (rv$salaries %>% filter(id == sid) %>% pull(is_edited) %>% .[1])
      }
    )
  ...
  observeEvent(input$salary_action_btn, {
    ...
    if (action == "edit") {
      ...
      # For default salaries, get the fresh default; for custom, just unmark as deleted
      defaults <- make_default_salaries()
      default_row <- defaults %>% filter(id == sid)
  ...
  output$value_unit_ui <- renderUI({
    ...
    # When editing, use the loaded value; otherwise use current input
    current_unit <- if (!is.na(rv$editing_id)) rv$value_unit_draft else (input$value_unit %||% "month")
    if (mode == "function" || mode == "sum") {
      ...
      # Variable mode: allow both, use current value as selected
      radioButtons("value_unit", "Input amount per", choices = c("Month" = "month", "Year" = "year"), selected = current_unit, inline = TRUE)
    }
  })
  ...
  # Update numeric_input_mode draft and refresh when user toggles single/multiple
  observeEvent(input$numeric_input_mode, {
    rv$numeric_input_mode_draft <- input$numeric_input_mode
    rv$value_inputs_refresh <- rv$value_inputs_refresh + 1L
  }, ignoreInit = TRUE)

  # Update salary_selected draft
  observeEvent(input$salary_selected, {
    rv$salary_selected_draft <- input$salary_selected
  }, ignoreInit = TRUE)

  # Update salary_field_selected draft
  observeEvent(input$salary_field_selected, {
  ...
  observeEvent(input$apply_inflation, {
    rv$apply_inflation_draft <- input$apply_inflation
  }, ignoreInit = TRUE)

  # Update apply_inflation default when value_mode changes (for adding new posts)
  observeEvent(input$value_mode, {
    if (is.na(rv$editing_id) && !is.null(input$value_mode)) {
      default_checked <- identical(input$value_mode, "salary")
      updateCheckboxInput(session, "apply_inflation", value = default_checked)
    }
  }, ignoreInit = TRUE)
  ...
  output$value_inputs_ui <- renderUI({
    ...
    if (mode == "function") {
      tagList(
        textInput("function_expr", amend_label("Formula expression", show_amend = is_amended_field("function_expr")), value = isolate(rv$function_expr_draft), placeholder = "Expression using n, FTE, fte(site), salary_identifier$total and apply_inflation() helper"),
      ...
      # Salary mode - select salary and field
      available_salaries <- sort(rv$salaries$name[!rv$salaries$is_deleted])
      ...
      # Numeric mode (formerly Variable) - single or multiple values
      # When editing, use the loaded unit; when creating, use input
      val_unit <- if (!is.na(rv$editing_id)) rv$value_unit_draft else input$value_unit
      
      # Calculate n_vals based on the correct unit
      req(input$post_date_range)
      req(length(input$post_date_range) == 2)
      req(all(!is.na(input$post_date_range)))
      range_start <- as.Date(input$post_date_range[1])
      range_end <- as.Date(input$post_date_range[2])
      n_vals <- if (range_end < range_start) {
        0L
      } else if (val_unit == "year") {
        n_years_between(range_start, range_end)
      } else {
        n_months_between(range_start, range_end)
      }
      ...
      if (numeric_mode == "single") {
        ...
        tagList(
          radioButtons("numeric_input_mode", "Input type:", choices = c("Single value (repeated)" = "single", "Multiple values" = "multiple"), selected = "single", inline = TRUE),
          numericInput("var_value_single", paste("Value to repeat for each", ifelse(val_unit == "year", "year", "month")), value = single_value, step = 100)
        )
      } else {
        tagList(
          radioButtons("numeric_input_mode", "Input type:", choices = c("Single value (repeated)" = "single", "Multiple values" = "multiple"), selected = "multiple", inline = TRUE),
          helpText(paste("Provide", n_vals, ifelse(val_unit == "year", "yearly", "monthly"), "amounts.")),
          lapply(seq_len(n_vals), function(i) {
            unit_label <- ifelse(val_unit == "year", "Year", "Month")
            numericInput(
  ...
  # Helper function to get available salary fields with display names
  get_available_salary_fields <- function() {
    c(
      "Base (monthly)" = "base",
      "Pension (monthly)" = "pension",
      "Own pension (monthly)" = "own_pension",
      "Holiday base (monthly)" = "holiday_base",
      "Holiday (monthly)" = "holiday",
      "Total with holiday (monthly)" = "total_plus_holiday",
      "Total with holiday (yearly)" = "total_plus_holiday_y",
      "Total (monthly)" = "total",
      "Total (yearly)" = "total_y"
    )
  }

  output$apply_inflation_ui <- renderUI({
    value_mode <- input$value_mode
    # Only show for salary, formula, and numeric modes
    if (is.null(value_mode) || !(value_mode %in% c("salary", "function", "variable"))) {
      return(NULL)
    }
    
    # Determine default: TRUE for salary, FALSE for others
    default_checked <- if (identical(value_mode, "salary")) TRUE else FALSE
    
    # Get current value from draft (when editing) or input (when adding)
    current_value <- if (!is.na(rv$editing_id)) rv$apply_inflation_draft else (input$apply_inflation %||% default_checked)
    
    checkboxInput("apply_inflation", "Apply inflation", value = current_value)
  })
  ...
  output$visualization_section <- renderUI({
    ...
    # Build list of tabs to show (only those with data)
    tabs_to_show <- list()
    
    if (nrow(visualization_data_applied_for()) > 0) {
      tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel("Applied for", plotlyOutput("viz_applied_for", height = "600px", width = "100%"))
    }
    if (nrow(visualization_data_funded()) > 0) {
      tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel("Funded", plotlyOutput("viz_funded", height = "600px", width = "100%"))
    }
    if (nrow(visualization_data_applied_elsewhere()) > 0) {
      tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel("Applied elsewhere", plotlyOutput("viz_applied_elsewhere", height = "600px", width = "100%"))
    }
    if (nrow(visualization_data_not_applied()) > 0) {
      tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel("Not applied for", plotlyOutput("viz_not_applied", height = "600px", width = "100%"))
    }
    if (nrow(visualization_data_all()) > 0) {
      tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel("All", plotlyOutput("viz_all", height = "600px", width = "100%"))
    }
    
    # Analytics tab
    tabs_to_show[[length(tabs_to_show) + 1]] <- tabPanel(
      "Analytics",
      tags$div(
        style = "display: flex; flex-direction: column; gap: 30px; padding: 20px;",
        tags$div(
          style = "margin-top: 10px;",
          plotlyOutput("analytics_status_breakdown", height = "500px")
        ),
        tags$div(
          style = "margin-top: 10px;",
          plotlyOutput("analytics_budget_by_category", height = "500px")
        )
      )
    )
    
    n_tabs <- length(tabs_to_show)
    tab_width <- 100 / n_tabs
  ...
  output$post_table_container <- renderUI({
    ...
    } else if (isTRUE(input$display_form == "summary")) {
      tagList(
        div(style = "padding: 4px 0 6px;",
          checkboxInput("summary_show_fte", "Show FTE columns", value = FALSE)
        ),
        DTOutput("summary_form_table")
      )
    } else {
  ...
  # ---------------------------------------------------------------------------
  # build_summary_table_data
  # Returns a one-row-per-post table pivoted wide by period.
  # Columns (in order):
  ...
  build_summary_table_data <- function(period_choice, posts_data, budget_start, budget_end,
    ...
    if (!nrow(posts_data)) return(empty_result())
    ...
    if (!nrow(long)) return(empty_result())
    ...
    post_period <- combined %>%
      group_by(id, Period) %>%
      summarise(
        amount     = sum(value),
        period_fte = first(fte) * n() / 12,
        .groups = "drop"
      )
    ...
    post_meta <- posts_data %>%
      transmute(
        ...
        Category           = category,
        Note               = note,
        Period             = if_else(value_mode == "salary" | !is.na(fte) | grepl("salary", post_name, ignore.case = TRUE) | grepl("salary", category, ignore.case = TRUE), paste0(format(start_date, "%Y-%m-%d"), ":", format(end_date, "%Y-%m-%d")), NA_character_),
        needs_amendment    = needs_amendment,
        application_status = if_else(is.na(application_status), "Applied for", application_status)
      )
    ...
    fte_cols    <- paste0("FTE ", all_periods)

    amounts_wide <- post_period %>%
      select(id, Period, amount) %>%
      tidyr::pivot_wider(names_from = Period, values_from = amount, values_fill = NA_real_)

    fte_wide <- post_period %>%
      select(id, Period, period_fte) %>%
      tidyr::pivot_wider(names_from = Period, values_from = period_fte, values_fill = NA_real_) %>%
      rename_with(~ paste0("FTE ", .), .cols = -id)

    df <- post_meta %>%
      left_join(amounts_wide, by = "id") %>%
      left_join(fte_wide,     by = "id")

    for (col in period_cols) if (!col %in% names(df)) df[[col]] <- NA_real_
    ...
    post_status_totals <- post_period %>%
      ...
      group_by(id, application_status) %>%
      summarise(total = sum(amount), .groups = "drop")

    for (st in status_cols) {
      st_totals <- post_status_totals %>%
        filter(application_status == st) %>%
        select(id, total)
    ...
    grand_row <- grand_row[, names(df), drop = FALSE]
    rows_list <- c(rows_list, list(grand_row))

    result <- bind_rows(rows_list)

    # Drop Site; insert spacer columns; keep needs_amendment and application_status as last hidden columns
  ...
  output$summary_form_table <- renderDT({
    ...
    fte_cols    <- summ$fte_cols
    status_cols <- summ$status_cols

    if (!nrow(df)) {
      return(datatable(
        tibble(Message = "No posts to display. Add posts or adjust filters."),
        colnames = "", rownames = FALSE,
        options = list(dom = "t", paging = FALSE, searching = FALSE, ordering = FALSE)
      ))
    }
    ...
    dt <- datatable(
      ...
      options = list(
        ...
        rowCallback = JS(
          ...
          # data[0]=id, data[1]=Post name, data[data.length-2]=needs_amendment, data[data.length-1]=application_status
          "  var id         = data[0];",
          "  var pname      = typeof data[1] === 'string' ? data[1] : '';",
          "  var needsAmend = data[data.length - 2];  // second to last",
          "  var appStatus  = data[data.length - 1];  // last",
          ...
          "    $('td', row).text('');",
          "    $(row).addClass('summary-structural');",
          "  } else if (needsAmend === true || needsAmend === 1 || needsAmend === 'true') {",
          "    // Amendment needed: add badge-like indicator on the left of Post name",
          "    var $pnameCell = $('td:eq(0)', row);",
          "    $pnameCell.html('<span class=\"amend-badge\">⚠</span> ' + $pnameCell.html());",
        ...
        drawCallback = JS(
          "function() {",
          "  var api = this.api();",
          "  api.rows('.summary-structural').nodes().to$().css('cursor','default');",
          "}"
        )
      )
    )

    round_cols_present <- intersect(c(period_cols, fte_cols, status_cols), col_names)
    ...
    dt <- dt %>% htmlwidgets::onRender(
      "function(el, x) {
        ...
        $(el).on('click', 'tr', function() {
          ...
          if ($(this).hasClass('summary-structural')) {
            row.deselect();
          }
        });
      }"
  ...
  selected_post_id <- reactive({
    if (identical(input$display_form, "summary")) {
      sel <- input$summary_form_table_rows_selected
      if (!length(sel)) return(NA_integer_)
      # Re-build the summary data to get the id column
      summ <- build_summary_table_data(
        input$period_format, rv$posts,
        input$budget_start, input$budget_end,
        rv$salaries, input$inflation_pct,
      ...
      if (row_idx < 1 || row_idx > nrow(summ$data)) return(NA_integer_)
      post_id <- summ$data$id[row_idx]
      if (is.null(post_id) || is.na(post_id)) return(NA_integer_)
      return(as.integer(post_id))
    }
  ...
  observeEvent(input$edit_selected, {
    ...
    if (is.na(sid)) {
      ...
      } else if (identical(input$display_form, "summary")) {
        rv$form_error_text <- "Select a post row in Summary view to edit. Site header, total, and separator rows are not editable."
      }
      return()
    }
    ...
    rv$apply_inflation_draft <- as.logical(row$apply_inflation[[1]] %||% FALSE)
    ...
    # Determine selected mode: use value_mode directly from the stored post
    selected_mode <- as.character(row$value_mode[[1]] %||% "variable")
    
    updateSelectInput(session, "value_mode", selected = selected_mode)
    updateRadioButtons(session, "value_unit", selected = row$value_unit[[1]])
    updateRadioButtons(session, "numeric_input_mode", selected = rv$numeric_input_mode_draft)
    ...
    updateCheckboxInput(session, "apply_inflation", value = rv$apply_inflation_draft)
  ...
  observeEvent(input$delete_selected, {
    ...
    if (is.na(sid)) {
      if (identical(input$display_form, "wide")) {
        ...
        rv$form_error_text <- "Select a post row in Summary view to delete. Site header, total, and separator rows cannot be deleted."
      }
      return()
    }
  ...
  observeEvent(input$make_inactive, {
    ...
    if (is.na(sid)) {
      if (identical(input$display_form, "wide")) {
        ...
        rv$form_error_text <- "Select a post row in Summary view to make inactive. Site header, total, and separator rows are not selectable."
      }
      return()
    }
    row_to_deactivate <- rv$posts %>% filter(id == sid)
    if (nrow(row_to_deactivate) == 0) {
      rv$form_error_text <- "Post not found or already inactive."
      return()
    }
    rv$posts <- rv$posts %>% filter(id != sid)
  ...
  observeEvent(input$activate_post_id, {
    ...
    if (nrow(row_to_activate) == 0) {
      rv$form_error_text <- "Post not found in inactive posts."
      return()
    }
  ...
  observeEvent(input$add_or_update, {
    ...
    vec <- numeric()
    val_mode <- if (is_editing) rv$value_mode_draft else input$value_mode
    if (isTRUE(val_mode == "variable")) {
      n_vals <- required_value_count()
      numeric_mode <- if (is_editing) rv$numeric_input_mode_draft else (input$numeric_input_mode %||% "multiple")
      
      if (numeric_mode == "single") {
        # Single value mode: repeat the one value for all periods
        single_val <- as.numeric(input$var_value_single %||% 0)
        vec <- rep(single_val, n_vals)
      } else {
        # Multiple values mode: get individual values
    ...
    # For salary mode, keep salary_selected/salary_field_selected; formula generated at calculation time
    ...
    apply_inflation_flag <- if (is_editing) rv$apply_inflation_draft else (input$apply_inflation %||% FALSE)
    
    # Handle salary mode: keep value_mode as "salary", don't store formula
    actual_value_mode <- val_mode
    if (val_mode == "salary") {
      if (!nzchar(sal_selected) || !nzchar(sal_field)) {
        rv$form_error_text <- "Salary mode requires both salary name and field to be selected."
        return()
      }
      # Validate that the selected salary exists
      salary_row <- rv$salaries %>% filter(name == sal_selected, !is_deleted)
      if (!nrow(salary_row)) {
        rv$form_error_text <- "Selected salary not found or has been deleted."
        return()
      }
      # Keep value_mode as "salary" (not converting to "function")
      actual_value_mode <- "salary"
    }
    ...
    new_row <- tibble(
      ...
      value_mode = actual_value_mode,
      value_unit = if (is_editing) rv$value_unit_draft else input$value_unit,
      constant_expr = rv$constant_expr_draft,
      # For salary mode, keep function_expr empty (formula generated at calculation time)
      # For function mode, keep user's formula as-is (inflation handled at calculation time)
      function_expr = if (actual_value_mode == "salary") "" else (if (is.null(input$function_expr) || !nzchar(input$function_expr)) rv$function_expr_draft else input$function_expr),
      ...
      apply_inflation = apply_inflation_flag,
  ...
  # Individual status-filtered visualization reactives
  create_status_filtered_viz <- function(target_status) {
    reactive({
      ...
      if (!nrow(long_df)) {
        return(tibble(month = as.Date(character()), center = character(), category = character(), value = numeric(), facet = character()))
      }
      ...
      status_ids <- rv$posts %>% filter(application_status == target_status) %>% pull(id)
      ...
      complete_grid <- expand_grid(month = all_months, center = all_centers, category = all_categories)
  ...
  visualization_data_applied_for <- create_status_filtered_viz("Applied for")
  visualization_data_funded <- create_status_filtered_viz("Funded")
  visualization_data_applied_elsewhere <- create_status_filtered_viz("Applied for elsewhere")
  visualization_data_not_applied <- create_status_filtered_viz("Not applied for")
  
  # All statuses combined (no status filter)
  visualization_data_all <- reactive({
    long_df <- build_long_budget(rv$posts, input$budget_start, input$budget_end, salaries_lookup = make_salary_lookup(rv$salaries), inflation_pct = input$inflation_pct, salaries_tbl = rv$salaries)
    
    if (!nrow(long_df)) return(tibble(month = as.Date(character()), center = character(), category = character(), value = numeric(), facet = factor(character())))
    ...
    # Aggregate by month, center, category (no status filter)
    agg_monthly <- long_df %>%
      group_by(month, center, category) %>%
      summarise(value = sum(value, na.rm = TRUE), .groups = "drop")
    ...
    agg_data <- complete_grid %>%
      ...
      ungroup() %>%
      mutate(facet = center, value = cumsum_value) %>%
      select(month, center, category, value, facet)
    
    # Add "All sites" row (computed from raw agg_monthly, ensuring complete grid)
    agg_all_sites_monthly <- agg_monthly %>%
      group_by(month, category) %>%
      summarise(value = sum(value, na.rm = TRUE), .groups = "drop")
    
    all_sites_complete_grid <- expand_grid(month = all_months, category = all_categories)
    
    all_sites_raw <- all_sites_complete_grid %>%
      left_join(agg_all_sites_monthly, by = c("month", "category")) %>%
      replace_na(list(value = 0)) %>%
      arrange(category, month) %>%
      group_by(category) %>%
      mutate(value = cumsum(value)) %>%
  ...
  # Analytics data reactives
  analytics_funding_by_status <- reactive({
    posts_with_status <- rv$posts %>% filter(value_mode != "sum") %>% select(id, center, application_status)
    long_df <- build_long_budget(rv$posts, input$budget_start, input$budget_end, salaries_lookup = make_salary_lookup(rv$salaries), inflation_pct = input$inflation_pct, salaries_tbl = rv$salaries)
    
    if (!nrow(long_df)) return(tibble(center = character(), status = character(), total_value = numeric()))
    ...
    long_df %>%
      left_join(posts_with_status %>% select(id, application_status), by = "id") %>%
      group_by(center, application_status) %>%
      summarise(total_value = sum(value, na.rm = TRUE), .groups = "drop") %>%
      replace_na(list(application_status = "Unknown")) %>%
      arrange(center, application_status)
  })

  analytics_budget_by_site <- reactive({
    long_df <- build_long_budget(rv$posts, input$budget_start, input$budget_end, salaries_lookup = make_salary_lookup(rv$salaries), inflation_pct = input$inflation_pct, salaries_tbl = rv$salaries)
    
    if (!nrow(long_df)) return(tibble(center = character(), total_value = numeric()))
  ...
  analytics_budget_by_category <- reactive({
    long_df <- build_long_budget(rv$posts, input$budget_start, input$budget_end, salaries_lookup = make_salary_lookup(rv$salaries), inflation_pct = input$inflation_pct, salaries_tbl = rv$salaries)
    
    if (!nrow(long_df)) return(tibble(category = character(), total_value = numeric()))
  ...
  analytics_metrics <- reactive({
    long_df <- build_long_budget(rv$posts, input$budget_start, input$budget_end, salaries_lookup = make_salary_lookup(rv$salaries), inflation_pct = input$inflation_pct, salaries_tbl = rv$salaries)
    
    if (!nrow(long_df)) return(tibble(metric = character(), value = numeric()))
    ...
    posts_with_status <- rv$posts %>% filter(value_mode != "sum") %>% select(id, application_status)
    data_with_status <- long_df %>%
      left_join(posts_with_status %>% select(id, application_status), by = "id") %>%
      replace_na(list(application_status = "Unknown"))
    
    total_budget <- sum(data_with_status$value, na.rm = TRUE)
    applied_for <- sum(data_with_status$value[data_with_status$application_status == "Applied for"], na.rm = TRUE)
    funded <- sum(data_with_status$value[data_with_status$application_status == "Funded"], na.rm = TRUE)
    applied_elsewhere <- sum(data_with_status$value[data_with_status$application_status == "Applied for elsewhere"], na.rm = TRUE)
    not_applied <- sum(data_with_status$value[data_with_status$application_status == "Not applied for"], na.rm = TRUE)
    
    tibble(
      metric = c("Total Budget", "Applied for", "Funded", "Applied Elsewhere", "Not Applied", "Funding Gap"),
      value = c(total_budget, applied_for, funded, applied_elsewhere, not_applied, applied_for - funded)
    )
  })

  # Helper function to create cumulative line chart for a given visualization_data reactive
  create_cumulative_line_chart <- function(viz_data) {
    if (!nrow(viz_data)) {
      p <- ggplot() +
        geom_blank() +
    ...
    plots <- lapply(facets, function(f) {
      ...
      period_months <- as.integer(difftime(as.Date(input$budget_end), as.Date(input$budget_start), units = "days") / 30)
  ...
  # Status-specific visualization renderers
  output$viz_applied_for <- renderPlotly({
    create_cumulative_line_chart(visualization_data_applied_for())
  })
  outputOptions(output, "viz_applied_for", suspendWhenHidden = FALSE)

  output$viz_funded <- renderPlotly({
    create_cumulative_line_chart(visualization_data_funded())
  })
  outputOptions(output, "viz_funded", suspendWhenHidden = FALSE)

  output$viz_applied_elsewhere <- renderPlotly({
    create_cumulative_line_chart(visualization_data_applied_elsewhere())
  })
  outputOptions(output, "viz_applied_elsewhere", suspendWhenHidden = FALSE)

  output$viz_not_applied <- renderPlotly({
    create_cumulative_line_chart(visualization_data_not_applied())
  })
  outputOptions(output, "viz_not_applied", suspendWhenHidden = FALSE)

  output$viz_all <- renderPlotly({
    create_cumulative_line_chart(visualization_data_all())
  })
  outputOptions(output, "viz_all", suspendWhenHidden = FALSE)

  # Analytics chart renderers
  output$analytics_status_breakdown <- renderPlotly({
    data <- analytics_funding_by_status()
    if (!nrow(data)) {
      p <- ggplot() + geom_blank() + theme_minimal()
      return(ggplotly(p, tooltip = "none"))
    }

    p <- ggplot(data, aes(x = center, y = total_value, fill = application_status)) +
      geom_col(position = "stack") +
      labs(title = "Budget by Funding Status", x = "Site", y = "Total Budget", fill = "Status") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    
    ggplotly(p, tooltip = c("x", "y", "fill"))
  })
  outputOptions(output, "analytics_status_breakdown", suspendWhenHidden = FALSE)

  output$analytics_budget_by_site <- renderPlotly({
    data <- analytics_budget_by_site()
    if (!nrow(data)) {
      p <- ggplot() + geom_blank() + theme_minimal()
      return(ggplotly(p, tooltip = "none"))
    }

    p <- ggplot(data, aes(x = reorder(center, total_value), y = total_value, fill = center)) +
      geom_col() +
      coord_flip() +
      labs(title = "Total Budget by Site", x = "Site", y = "Total Budget") +
      theme_minimal() +
  ...
  output$analytics_budget_by_category <- renderPlotly({
    data <- analytics_budget_by_category()
    if (!nrow(data)) {
      p <- ggplot() + geom_blank() + theme_minimal()
      return(ggplotly(p, tooltip = "none"))
    }

    p <- ggplot(data, aes(x = reorder(category, total_value), y = total_value, fill = category)) +
      geom_col() +
      coord_flip() +
      labs(title = "Total Budget by Category", x = "Category", y = "Total Budget") +
      theme_minimal() +
      theme(legend.position = "none")
    
    ggplotly(p, tooltip = c("x", "y"))
  })
  outputOptions(output, "analytics_budget_by_category", suspendWhenHidden = FALSE)

  output$analytics_metrics <- renderUI({
    ...
    metrics_html <- sapply(seq_len(nrow(data)), function(i) {
      metric <- data$metric[i]
      value <- data$value[i]
      value_formatted <- scales::dollar(value)
      paste0(
        '<div style="padding: 10px; margin: 5px; border: 1px solid #ddd; border-radius: 5px; background: #f9f9f9;">',
  ...
  observeEvent(input$show_formula_help, {
    showModal(modalDialog(
      ...
      tags$ul(
        tags$li("Each salary identifier is available as an object, e.g. phd_student$total"),
        tags$li("Identifiers are generated from salary names and made unique"),
        tags$li("Selectors: base, pension, own_pension, holiday_base, holiday, total_plus_holiday, total"),
        tags$li(tags$b("total"), " — the final effective monthly salary (holiday-absence deducted if 'Subtract holiday' is on)"),
        tags$li(tags$b("total_plus_holiday"), " — monthly salary including holiday allowance but before holiday-absence deduction")
      ),
      tags$p("Examples:"),
      tags$pre("apply_inflation(phd_student$total)\nresearch_year$total_plus_holiday\nclinical_researcher$total")
    ))
  })

  # Parameterized versions that accept data as arguments (for use in deferred execution)
  build_export_sheet_with_data <- function(period_choice, posts_data, budget_start, budget_end, salaries_data, inflation_pct, squash_dims) {
    if (!nrow(posts_data)) {
      return(tibble(
        Period = character(),
        `Post name` = character(),
    ...
    if ("Post name" %in% squash_dims) out$`Post name` <- "All posts"
    if ("Site" %in% squash_dims) out$Site <- "All sites"
    if ("Category" %in% squash_dims) out$Category <- "All categories"
    if ("Post name" %in% squash_dims) out$Note <- "Mixed notes"
  ...
  build_export_summary_sheet_with_data <- function(period_choice, posts_data, budget_start, budget_end,
                                                    salaries_data, inflation_pct) {
    ...
    result <- d %>% 
      select(-id, -needs_amendment, -application_status) %>%
      rename(`Employment period` = Period)
  ...
  do_export_now <- function() {
    ...
    # Snapshot all data SYNCHRONOUSLY while in reactive context
    posts_snapshot <- isolate(rv$posts)
    salaries_snapshot <- isolate(rv$salaries)
    site_registry_snapshot <- isolate(rv$site_registry)
    category_registry_snapshot <- isolate(rv$category_registry)
    template_registry_snapshot <- isolate(rv$template_registry)
    budget_start_snapshot <- isolate(input$budget_start)
    budget_end_snapshot <- isolate(input$budget_end)
    inflation_pct_snapshot <- isolate(input$inflation_pct)
    squash_dims_snapshot <- isolate(input$squash_dims)

    # Set progress flag IMMEDIATELY (still synchronously) so Shiny queues the UI update
    rv$export_in_progress <- TRUE
    export_filename <- paste0(export_base_name, ".xlsx")

    # NOW defer the heavy work so the progress bar update gets sent to browser FIRST
    later::later(function() {
      tryCatch({
        # Generate salary identifiers for existing salaries
        salaries_copy <- salaries_snapshot
        if (nrow(salaries_copy) > 0) {
          salary_identifiers <- character()
          for (i in seq_len(nrow(salaries_copy))) {
            if (is.na(salaries_copy$identifier[i]) || !nzchar(salaries_copy$identifier[i])) {
              salary_identifiers[i] <- generate_salary_identifier(salaries_copy$name[i], salary_identifiers)
            } else {
              salary_identifiers[i] <- salaries_copy$identifier[i]
            }
          }
          salaries_copy$identifier <- salary_identifiers
        }
        ...
        sheets <- list(
          ...
          sites                 = to_js_sheet(serialize_sites(site_registry_snapshot)),
          categories            = to_js_sheet(serialize_categories(category_registry_snapshot)),
          templates             = to_js_sheet(serialize_templates(template_registry_snapshot)),
          meta                  = to_js_sheet(tibble(
            key = c("budget_start", "budget_end", "inflation_pct", "workbook_name", "schema_version"),
            value = c(as.character(budget_start_snapshot), as.character(budget_end_snapshot), as.character(inflation_pct_snapshot), export_base_name, "2.0")
          ))
        )

        # Send download and clear progress
        session$sendCustomMessage("download-xlsx", list(
          filename = export_filename,
          sheets = sheets
        ))
        set_success(paste("Export started:", export_filename))
        rv$export_in_progress <- FALSE

      }, error = function(e) {
        rv$export_error_text <- paste("Export error:", e$message)
        rv$export_in_progress <- FALSE
      })
    }, delay = 0.05)
  }
